...
const TicketSummary = () => {
  const { selectedTickets, totalEUR, ticketAssignments, attendees } = useTicketContext();
  ...
  const getAttendeesByTicket = () => {
    const attendeesByTicket: { [ticketIndex: number]: number[] } = {};
    Object.entries(ticketAssignments).forEach(([attendeeIndex, ticketIndex]) => {
      if (!attendeesByTicket[ticketIndex]) {
        attendeesByTicket[ticketIndex] = [];
      }
      attendeesByTicket[ticketIndex].push(parseInt(attendeeIndex));
    });
    return attendeesByTicket;
  };

  const attendeesByTicket = getAttendeesByTicket();

  const getUnassignedAttendees = () => {
    const assignedAttendees = new Set(Object.keys(ticketAssignments).map(Number));
    const allAttendees = Array.from({ length: totalAttendees }, (_, i) => i);
    return allAttendees.filter(i => !assignedAttendees.has(i));
  };

  const unassignedAttendees = getUnassignedAttendees();
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {ticketSummary.map((item, index) => {
                      const assignedCount = attendeesByTicket[item.index]?.length || 0;
                      return (
                          ...
                          <div className="mt-3 pt-3 border-t border-gray-100">
                            <div className="flex justify-between text-sm">
                              <span className="text-gray-600">
                                Assigned: {assignedCount} / {item.quantity}
                              </span>
                              <span
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {ticketSummary.map((item) => (
                      attendeesByTicket[item.index] && attendeesByTicket[item.index].length > 0 && (
                        <div key={item.index} className="border border-gray-200 rounded-lg p-4">
                          <h4 className="font-medium text-gray-700 mb-2">{item.title}</h4>
                          <div className="space-y-2">
                            {attendeesByTicket[item.index].map((attendeeIndex) => {
                              const attendee = attendees[attendeeIndex];
                              return (
                                <div key={attendeeIndex} className="border-b border-gray-100 pb-2">
                                  ...
                                  {attendee && hasAttendeeDetails(attendee) ? (
                                    <div className="mt-1 text-sm text-gray-600">
                                      ...
                                      <p>
                                        ...
                                        {attendee.emailAddress || 'Not provided'}
                                      </p>
                                    </div>
                                  ) : (
                    ...
                    {unassignedAttendees.length > 0 && (
                      <div className="border border-red-200 rounded-lg p-4 bg-red-50">
                        ...
                        <div className="space-y-2">
                          {unassignedAttendees.map((attendeeIndex) => {
                            const attendee = attendees[attendeeIndex];
                            return (
                              <div key={attendeeIndex} className="border-b border-gray-100 pb-2">
                                <div className="flex items-center justify-between text-sm">
                        ...
                        <p className="text-xs text-red-600 mt-2">
                          These attendees need to select their ticket type during registration.
                        </p>
                      </div>
                    )}
                  </div>
                  <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-xs text-gray-600 space-y-1">
                      ...
                      <div className="flex justify-between">
                        <span>Assigned:</span>
                        <span>{Object.keys(ticketAssignments).length}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Unassigned:</span>
                        <span className={unassignedAttendees.length > 0 ? 'text-red-600 font-medium' : ''}>
              ...
              <div className="flex gap-3">
                {unassignedAttendees.length > 0 && (
                  <div className="flex items-center text-sm text-red-600 mr-4">
                    <span className="w-2 h-2 bg-red-500 rounded-full mr-2"></span>
                    {unassignedAttendees.length} attendee(s) unassigned
                  </div>
                )}
                <button
